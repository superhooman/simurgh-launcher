...
class Handler {
		...
		return new Promise(resolve => {
			...
			file.on('error', async e => {
				...
				if (fs.existsSync(path.join(directory, name))) {
					shelljs.rm(path.join(directory, name));
				}
	...
	getAssets() {
		return new Promise(async resolve => {
			...
			await Promise.all(Object.keys(index.objects).map(async asset => {
				const {hash} = index.objects[asset];
	...
	async getForgeDependenciesLegacy() {
		...
		try {
			...
			this.client.emit('debug', `[MCLC]: Unable to extract version.json from the forge jar due to ${error}`);
			return null;
		}
		...
		await Promise.all(forge.libraries.map(async library => {
			...
			if (!download) {
				await this.downloadAsync(`${this.options.overrides.url.fallbackMaven}${lib[0].replace(/\./g, '/')}/${lib[1]}/${lib[2]}/${name}`, jarPath, name, true, 'forge');
			}
	...
	extractPackage(options = this.options) {
		return new Promise(async resolve => {
			...
			if (options.removePackage) {
				shelljs.rm(options.clientPackage);
			}
